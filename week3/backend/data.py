...
# Make week3/ importable so we can reach the sibling `validation/` package.
_WEEK3_DIR = Path(__file__).resolve().parent.parent  # .../week3
if str(_WEEK3_DIR) not in sys.path:
    sys.path.insert(0, str(_WEEK3_DIR))

from validation.check_data_quality import validate_and_log

# Emit to stdout so container platforms (Docker, GKE) capture it automatically.
logging.basicConfig(
    level=logging.INFO,
...
LOOKUP_PATH = _ROOT / "week2" / "metadata" / "Lookups" / "taxi_zone_lookup_real.csv"
...
# --- Week 3: incoming-data quality check -----------------------------------
# Path to this week's incoming batch (the "corrupted" upstream data).
# Kept separate from the clean Week 2 model data — this is only checked,
# never fed to the model.
INCOMING_DATA_PATH = _ROOT / "week3" / "data" / "demand_enriched_corrupted.parquet"

# Time boundary between the clean historical part of the file and the freshly
# arrived batch. Rows before this date are trusted reference (baseline); rows
# from this date onward are the batch we actually validate. Validating only the
# new window is what production would do — you check what just landed, not the
# whole history each run. It also lets a feature that collapses *inside* the new
# window be caught even though the full file still looks varied.
INCOMING_WINDOW_START = pd.Timestamp("2026-01-16")


def check_and_log_data_quality():
    """Run validation on the incoming data batch and log any issues.

    Graceful degradation contract:
      - The API never crashes because of validation problems.
      - Anything detected is logged at WARNING or above so operators see it.
      - If the file is missing or validation itself blows up, we log and move on.
    """
    try:
        df = pd.read_parquet(INCOMING_DATA_PATH)
    except FileNotFoundError:
        logger.warning(
            "Incoming data file not found at %s — skipping data quality check.",
            INCOMING_DATA_PATH,
        )
        return
    except Exception as exc:
        logger.error("Could not read incoming data for validation: %s", exc)
        return

    try:
        ts = pd.to_datetime(df["time_bucket"])
        baseline = df[ts < INCOMING_WINDOW_START]
        incoming = df[ts >= INCOMING_WINDOW_START]
    except Exception as exc:
        logger.error("Could not split incoming data by time window: %s", exc)
        return

    validate_and_log(incoming, baseline=baseline, log=logger)
